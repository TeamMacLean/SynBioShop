...
uploadController.uploadSequenceFile = async (req, res) => {
    ...
    const newPath = path.join(config.uploadRoot, seqFile.name); // Using config.uploadRoot for simplicity

    try {
        // processFileUpload handles mkdirp and fs.rename
        await processFileUpload(seqFile, newPath, 'SequenceFile', itemID);
...
uploadController.deleteSequenceFile = async (req, res) => {
    ...
    const fileIdToDelete = req.body.sequenceFileID || req.params.id || req.query.id;

    if (!fileIdToDelete) {
        console.error('ERROR: No valid file ID could be extracted for deletion. Preventing DB query.');
    ...
    try {
        ...
        if (!sequenceFile) {
            console.warn(`WARNING: Sequence file with ID ${fileIdToDelete} not found in DB.`);
            Flash.error(req, 'Sequence file not found or already deleted.');
            return res.redirect('back');
        }

        console.log(`Found sequence file: ${sequenceFile.originalName}, proceeding with deletion.`);
        // Optionally, delete the actual file from disk:
        // await fsPromises.unlink(sequenceFile.path);

        await sequenceFile.delete();
        console.log(`Successfully deleted sequence file ${sequenceFile.originalName} (ID: ${fileIdToDelete}).`);
