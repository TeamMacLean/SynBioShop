...
docs.rearrangeSave = (req, res) => {
  ...
  function process(obj) {
    ...
    if (!("documents" in obj) && !("subjects" in obj)) {
      toDo.push(update(Document, obj));
    } else {
      if (obj.subjects) {
        obj.subjects.map((o) => {
          process(o);
        });
        toDo.push(update(Subject, obj));
      }
      if (obj.documents) {
        obj.documents.map((o) => {
          process(o);
...
docs.subject.delete = (req, res) => {
  ...
  Subject.get(id)
    .then((subject) => {
      subject
        .deleteAll({ documents: true, subjects: true })
        .then(() => {
...
docs.document.delete = (req, res) => {
  ...
  Document.get(id)
    .then((document) => {
      if (!document) {
        return renderError(new Error("Document not found"), res);
      }
      // Store subjectID before deleting
      const subjectID = document.subjectID;
      document
        .delete()
        .then(() => {
          // Redirect to the subject page (not /docs/item/)
          return res.redirect(subjectID ? `/docs/${subjectID}` : "/docs");
        })
        .catch((err) => renderError(err, res));
    })
    .catch((err) => renderError(err, res));
      ...
      if (id) {
        ...
        doc
          ...
          .then((saved) => {
            ...
            getTopLevelSubjects()
              .then((subjects) => {
                return res.render("documents/item/show", {
                  document: saved,
                  subjects,
                });
              })
...
docs.document.edit = (req, res) => {
  ...
  Document.get(itemID)
    ...
    .then((document) => {
      getTopLevelSubjects()
        .then((subjects) => {
          return res.render("documents/item/edit", {
