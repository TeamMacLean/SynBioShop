...
async function processMapFile(savedType, req) {
  ...
  if (!fileName) {
    console.warn("Map file upload: No valid filename found", file);
    return Promise.resolve();
  }

  // Check if file has a valid path (from multer/formidable/express-fileupload)
  // express-fileupload stores the file in memory by default, so we need to use mv() method
  ...
  try {
    ...
    if (typeof file.mv === "function") {
      ...
      console.warn(
        "Map file upload: Cannot move file - no source path or mv method",
        file,
      );
      return Promise.resolve();
    }
    ...
    console.error(
      "Error processing map file for type " + savedType.id + ":",
      err,
    );
    throw err;
  }
      ...
      categories.map(async (category) => {
        ...
        const typeDefinition =
          category.db && category.db.type
            ? Type.getByTypeNumber(category.db.type)
            : null;

        const items = typesForCategory.map((t) => {
          const whoMadeIt = t.whoMadeIt || t.source || "";

          const rowData = [whoMadeIt, t.description || ""];
          if (typeDefinition) {
            typeDefinition.fields.forEach((fieldDef) => {
              if (
                !["whoMadeIt", "description", "source"].includes(fieldDef.name)
              ) {
    ...
    for (const db of dbs) {
      for (const category of db.categories) {
        try {
          ...
          console.error(
            "Failed to fetch types for category " + category.id + ":",
            err,
          ); // Changed to concatenate string
...
premadeController.category.show = async (req, res) => {
  ...
  try {
    ...
    if (!typeDefinition) {
      // Safely access category.db.type (replaces category.db?.type)
      throw new Error(
        "Type definition not found for DB type: " +
          ((category.db && category.db.type) || "(unknown)"),
      ); // Changed to concatenate string
    }
    ...
    const dbs = await getSortedDBs();
    res.render("premade/category/show", {
      db: category.db,
      dbs,
      headings,
      items,
...
premadeController.item.save = async (req, res) => {
  ...
  try {
    ...
    if (id) {
      ...
      // Convert concentration to number, default to 0 if empty or invalid
      typeInstance.concentration =
      ...
      if (linkurl && Array.isArray(linkurl)) {
        typeInstance.citations = linkurl
          .map((url, index) => ({
            url: url || "",
    ...
    if (err.name === "ValidationError") {
      console.error(
        "Validation Error during item save:",
        err.message,
        err.errors,
      );
      Flash.error(
...
premadeController.item.show = async (req, res) => {
  ...
  try {
    ...
    const typeDefinition =
      item.db && item.db.type ? Type.getByTypeNumber(item.db.type) : null;
    if (!typeDefinition) {
      throw new Error(
        "Type definition not found for item's DB type: " +
          ((item.db && item.db.type) || "(unknown)"),
      );
    }
    ...
    if (
      item.mapFile &&
      Array.isArray(item.mapFile) &&
      item.mapFile.length > 0
    ) {
      item.mapFile = item.mapFile.sort(
    ...
    if (req.user) {
      try {
        const cart = await Cart.filter({ username: req.user.username })
          .getJoin({ items: true })
          .run();
        if (cart && cart.length > 0 && cart[0].items) {
          isInCart = cart[0].items.some(
            (cartItem) => cartItem.typeID === item.id,
          );
        }
      } catch (cartErr) {
        console.error(
          "Error checking if item is in cart for premade/item/show:",
          cartErr,
        );
      }
    }
...
premadeController.item.edit = async (req, res) => {
  ...
  try {
    ...
    if (
      type.mapFile &&
      Array.isArray(type.mapFile) &&
      type.mapFile.length > 0
    ) {
      type.mapFile = type.mapFile.sort(
        (a, b) => new Date(b.createdAt) - new Date(a.createdAt),
      )[0];
    } else {
      type.mapFile = null;
    ...
    if (!category)
      throw new Error("Parent category not found for item editing."); // Added check
    const dbs = await getSortedDBs();
