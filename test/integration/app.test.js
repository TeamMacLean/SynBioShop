...
describe("Application Integration Tests", function () {
  ...
  describe("Protected Routes", function () {
    ...
    it("should handle /admin route", function (done) {
      request(app)
        .get("/admin")
        .end(function (err, res) {
          // Should either redirect, show unauthorized, or return 404 if route doesn't exist
          expect([200, 301, 302, 401, 403, 404]).to.include(res.status);
          done();
        });
    });
  });
  ...
  describe("Error Handling", function () {
    it("should return 404 for non-existent routes", function (done) {
      request(app).get("/this-route-does-not-exist-12345").expect(404, done);
    });
