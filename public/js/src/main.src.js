...
function initCart() {
  function flyToCart(el) {
    ...
    var gotoX =
      $(cart).offset().left + $(cart).width() / 2 - $(el).width() / divider / 2;
    var gotoY =
      $(cart).offset().top +
      $(cart).height() / 2 -
      $(el).height() / divider / 2;
  ...
  $('*[data-addToCart="true"]').on("click", function (e) {
    ...
    socket.emit("addToCart", {
      typeID: typeID,
      username: document.signedInUser.username,
    });
  });

  socket.on("addedToCart", function (type) {
    var elForItemAdded = $(
      '*[data-addToCart="true"][data-typeID="' + type.id + '"]',
    );
...
function initMicroModal() {
  try {
    // Only initialize if there are modal triggers on the page
    ...
    console.warn("MicroModal initialization failed:", err.message);
  }
}

function initInputmask() {
  // Only initialize if the element exists
  const costCodeEl = document.querySelector("#costCode");
  if (costCodeEl) {
    try {
      // Use Inputmask with proper configuration
      Inputmask().mask(costCodeEl);
    } catch (err) {
      // Silently fail if Inputmask has issues - don't break the page
      console.warn("Inputmask initialization failed:", err.message);
    }
  }
}
